Template.eventStatistics.helpers({
	'_statisticEvent': function() {
		...
		if (_group.event) {
			...
			_objReturn.all = _group.usersId.length + 1;
...
Template.eventStatistics.events({
	...
	'click div[name=status-group-raise]': function(e, tmp) {
		...
		var group = Groups.findOne({name: tmp.data.nameGroup});

		if(group.event && group.event.status === 1) {
			console.log('this use send email');
			Meteor.call('sendEmail', Meteor.userId(), tmp.data.nameGroup);
		}
